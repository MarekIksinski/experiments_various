...
to do:
    ...
    *button to clear conversation context (reset conversation)
"""
import os
import sys
import base64
...
# Main Application (UI is mostly unchanged, submit() method is refactored and reset functionality added)
class MultiFunctionApp(QWidget):
    def __init__(self):
        super().__init__()
        self.logger = Logger()
        # chat_history will hold a list of conversation messages
        self.chat_history = []
        # attached_files stores any text files attached for the current prompt
        self.attached_files = []
        self.init_ui()
    ...
    def init_ui(self):
        ...
        # Output display for model responses or status messages
        self.output_label = QLabel("Output:")
        self.output_entry = QTextEdit()
        
        # File selection buttons: one to attach an image and one to attach text files.
        ...
        # Submit button triggers the API call using current settings and conversation context.
        self.submit_button = QPushButton("Submit")
        self.submit_button.clicked.connect(self.submit)
        
        # New Reset Conversation button – clears conversation history
        ...
        # Left layout (inputs and controls)
        ...
        # Right layout (chat history display)
        self.right_layout = QVBoxLayout()
        self.right_layout.addWidget(self.chat_history_label)
        self.right_layout.addWidget(self.chat_history_display)

        # Main horizontal layout: left column for inputs and right column for chat history
    ...
    def select_image_files(self):
        ...
        # Place the list of filenames in the prompt area for reference.
    ...
    def attach_text_files(self):
        ...
        if filenames:
            # Save the attached file paths for later use.
    ...
    def submit(self):
        ...
        # Log a new conversation header if this is a fresh conversation.
        if not self.chat_history:
            self.logger.log_header(model)

        # If text files have been attached, append their content to the prompt.
        if self.attached_files:
            ...
            # Clear attached files after appending their content.
            self.attached_files = []
        ...
        try:
            if mode == "Chat":
                ...
                with requests.post(f"{OLLAMA_BASE_URL}/api/chat", json=payload, stream=True) as response:
                    ...
                    for line in response.iter_lines():
                        if line:
                            ...
                            # Update the output in real-time (streaming effect).
                ...
                formatted_history = "\n\n".join(
                    [f"User: {msg['content']}" if msg['role'] == "user" else f"Model: {msg['content']}"
                     for msg in self.chat_history]
                )
                self.chat_history_display.setText(formatted_history)
                self.output_entry.setText(response_content)

            else:  # Image to Text mode
                # In this mode, the prompt entry contains image file paths.
                filenames = prompt.splitlines()
                # Use a default text prompt; you can modify as needed.
    ...
    def reset_conversation(self):
        """Resets the conversation context by clearing chat history and attached files."""
        self.chat_history = []
        self.attached_files = []
        # Clear the chat display.
        self.chat_history_display.clear()
        # Optionally, notify the user that the conversation has been reset.
        self.output_entry.append("Conversation has been reset.")
